...
let dbData = { reviews: [], reservations: [] };
...
// Get bookings count for a specific date+time slot (accounting for 2hr duration)
function getBookingsForSlot(date, timeSlot) {
  const slotIndex = ALL_SLOTS.indexOf(timeSlot);
  return dbData.reservations.filter(r => {
    if (r.date !== date) return false;
    const rIndex = ALL_SLOTS.indexOf(r.time);
    // A 2-hour booking blocks the chosen slot and the next slot
    return Math.abs(rIndex - slotIndex) < SLOT_DURATION;
  }).length;
}

// Update time slot dropdown with availability
async function updateSlotAvailability() {
  ...
  resTimeSelect.innerHTML = '<option value="">Select a time</option>';

  ALL_SLOTS.forEach(slot => {
    const count = getBookingsForSlot(date, slot);
    const available = MAX_TABLES - count;
    ...
    if (available <= 0) {
      ...
      option.textContent = `${slot} — Only ${available} table${available > 1 ? 's' : ''} left!`;
      option.style.color = '#c0704a';
    } else {
      option.textContent = `${slot} — ${available} tables available`;
    }
    resTimeSelect.appendChild(option);
  });
}

resDateInput.addEventListener('change', updateSlotAvailability);
...
resBtn.addEventListener('click', async () => {
  ...
  resBtn.textContent = 'Checking availability...';
  resBtn.disabled = true;

  // Re-fetch to get latest data before booking
  await fetchDB();
  const count = getBookingsForSlot(date, time);

  if (count >= MAX_TABLES) {
    resMsg.style.color = '#c0392b';
    resMsg.textContent = 'Sorry, this slot just got fully booked! Please choose another time.';
  ...
  // Save reservation
  const newReservation = { name, email, date, time, guests, notes, bookedAt: new Date().toISOString() };
  ...
  try {
    ...
    await fetch('https://formspree.io/f/xjgdbevj', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
      body: JSON.stringify({ ...newReservation, type: 'Reservation' })
    });

    resMsg.style.color = '#c0704a';
    resMsg.textContent = `✅ Table confirmed for ${name} on ${date} at ${time} for ${guests} guest(s)! See you soon ☕`;
...
function getNextHappyHour() {
  ...
  if (day === 0 || day === 6 || now >= target) {
    ...
    while (true) {
      ...
      if (next.getDay() !== 0 && next.getDay() !== 6) { next.setHours(17,0,0,0); return next; }
      daysAhead++;
    }
  }
  return target;
...
function renderReviews() {
  ...
  rvCount.textContent = `${dbData.reviews.length} review${dbData.reviews.length !== 1 ? 's' : ''}`;
  if (sorted.length === 0) {
...
document.getElementById('rvSubmit').addEventListener('click', async () => {
  ...
  if (!name || !text || selectedStars === 0) {
    rvMsg.style.color = '#c0392b'; rvMsg.textContent = 'Please fill in your name, pick a star rating, and write a review.'; return;
  }
  document.getElementById('rvSubmit').textContent = 'Submitting...';
  document.getElementById('rvSubmit').disabled = true;
  await fetchDB();
  dbData.reviews.push({ name, text, rating: selectedStars, date: new Date().toISOString() });
  try {
    await saveDB();
    renderReviews();
    ...
    rvMsg.style.color = '#c0392b'; rvMsg.textContent = 'Could not save review. Please try again.';
    dbData.reviews.pop();
  }
